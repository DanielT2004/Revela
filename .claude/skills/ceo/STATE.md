...
6. [ ] **Notification pre-permission primer** (added 2026-07-21) — custom "I'll ping you the second your cut's ready" screen before the OS prompt (currently fires cold at analysis time in the coordinators). Lifts accept rates; closed-app push is the re-engagement path after the slow Gemini pass → serves fifth-video retention. Small; pair with the device weekend. (Onboarding "How Vela works" flow itself = DONE + frozen 2026-07-21.)
...
- **2026-07-21** — **Onboarding "How Vela works" flow — final paid-conversion pass shipped, now FROZEN.** Reframed from "AI replaces your work" → "the editor you know, that knows you" (kills black-box distrust — the #1 churn trigger; positions Vela as familiar-editor + moat). Informed by Mobbin's "1,460 onboarding flows" study: rebuilt page-0 hero as a before/after (crammed manual timeline ~2 hrs → Vela's clean cut ~2 min = hours-back made visual); each page now maps to a willingness-to-pay driver; added a **founder's note** beat before sign-up ("feel human" lever — trust in a solo-built indie tool right before the ask); every-cut-yours HITL framing made explicit; kept "no watermark/credits/full-res." Declined paywall-inside-onboarding (value-before-ask; ours lands after first export). Build green; device test pending. **CEO ruling: onboarding is now frozen** (5 iterations = enough; burnout/moat-procrastination risk) — reopens only if a Phase-1 tester is visibly confused. **NEW next item before the gate checklist: notification pre-permission primer** ("I'll ping you the second your cut's ready" before the OS prompt fires cold at analysis time — the study shows a primer lifts accept rates significantly, and closed-app push is how Vela re-engages after the slow Gemini pass → directly serves fifth-video retention). Small plumbing item; do before/with the device-test weekend.
