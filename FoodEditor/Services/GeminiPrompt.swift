...
enum GeminiPrompt {
    ...
    - keep: true if this segment should appear in the final edit. Set to false only if it is redundant, a repeated take, off-topic, or adds nothing visually or informationally. Never drop a segment merely to speed up pacing — keep complete thoughts intact. A segment marked keep: false must never appear in final_edit_order.
    ...
    - section: which part of the FINAL video this segment belongs to — exactly one of "intro", "middle", or "end". "intro" = the opening context that sets the scene: where they are, the restaurant name, what they ordered, how long they waited, plus any establishing/ambiance shot of the place AND the hook. "middle" = the main content: tasting, bite reactions, dish close-ups, describing the food. "end" = the wrap: the verdict/rating, a final beauty shot, any sign-off. KEEP the intro's spoken context and establishing shots — do NOT drop them as filler or "slow"; introducing the place is essential to a food review, not dead air. [STYLE-AWARE: if the style block provides a section map, use it to decide which beats belong in each section and which to keep.]

    - final_edit_order: an array of segment IDs in the exact sequence they should appear in the final video. GROUP IT BY SECTION in order: every "intro" segment first (led by the strongest hook, then the spoken setup), then "middle", then "end". Only include segment IDs where keep is true. This is the final edit sequence a video editor would follow. [STYLE-AWARE: within that intro→middle→end shape, follow the creator's typical arc and end on their typical closing style, as far as the footage allows.]

    - broll_placements: a SHORT, intentional list of B-roll overlays — moments where a food close-up briefly covers the speaker's face while their AUDIO KEEPS PLAYING. Be sparse and deliberate, like a real editor: each placement covers a short sub-range (usually 1.5 to 3 seconds), and most of the talking should stay face-on — do NOT blanket the whole video with b-roll. For each placement: over_segment_id is the talking segment whose face you cover (a segment with keep: true whose scene_type is "talking-head"); broll_segment_id is the segment whose VISUAL you show over it — pick a clip that shows the FOOD, not a face: a "food-closeup", a "plating" pour/cook/assembly shot, or another strong food visual (it must be a DIFFERENT, kept segment and must NOT be a "talking-head"); start_offset_seconds is how many seconds INTO the over-segment the overlay begins (0 = the segment's start); duration_seconds is how long it covers. Choose each B-roll to MATCH what is being said at that moment — when they mention the sauce, show the sauce; when they mention the crust, show the crust. VARY the shots: never reuse the same broll_segment_id back-to-back. You MAY place a B-roll over the opening hook if it makes the open stronger. All times are in the same merged-video seconds as the segment start_seconds/end_seconds. If no placement is clearly motivated, return an empty array — never force b-roll just to fill space. [STYLE-AWARE: if a style block gives a b-roll coverage target, aim for roughly that fraction of the final video covered by these overlays, and prefer the creator's favored b-roll shot types.]
    ...
    1. Capture what makes THIS creator distinctive. Do not flatten them into generic categories. Wherever a preset option doesn't fit what you actually see, use the provided open text field instead of forcing a wrong label. It is better to describe accurately in your own words than to pick the closest preset.
